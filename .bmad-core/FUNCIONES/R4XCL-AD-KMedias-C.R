  ...
  
{
  ...
  # Función auxiliar para manejar tablas grandes siguiendo patrón BMad
  manejar_tabla_grande <- function(data, nombre_tabla) {
    if (nrow(data) > 50) {
      # Mostrar solo primeros 10 registros
      resultado_limitado <- head(data, 10)
      
      # Agregar fila informativa con instrucciones de exportación
      fila_info <- data.frame(
        "Cluster_Asignado" = paste0(">>> TABLA COMPLETA: ", nrow(data), " registros. Use AD_KMedias_ExportarCSV() para exportar <<<")
      )
      
      # Combinar datos limitados con información
      resultado_final <- rbind(resultado_limitado, fila_info)
  ...
  if (TipoOutput <= 0){
    ...
    A      <- res.kmns$cluster
    tabla_resultado <- data.frame("Cluster_Asignado" = A)
    OutPut <- manejar_tabla_grande(tabla_resultado, "Cluster Asignado")
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++    
# FUNCION DE EXPORTACION CSV SIGUIENDO PATRON BMAD     +
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++

AD_KMedias_ExportarCSV <- function(
    SetDatosX,
    archivo_destino = "clusters_kmeans.csv",
    Escala=0,
    Filtro=0,
    K=3,
    Semilla=123456,
  ...
  datos_completos <- data.frame("Cluster_Asignado" = res.kmns$cluster)
  
  # Exportar a CSV
  write.csv(datos_completos, archivo_destino, row.names = FALSE)
  
  return(paste0("Exportación exitosa: ", archivo_destino, " (", nrow(datos_completos), " registros)"))
}
...
attr(AD_KMedias_ExportarCSV, "description") = 
  list(
    Detalle         = "Exportar resultados completos de K-Medias a CSV",
    SetDatosX       = DialogosXCL$SetDatosX.NS,
    archivo_destino = "Nombre del archivo CSV (ej: 'mis_clusters.csv')",
    Escala          = DialogosXCL$Escala,
    Filtro          = DialogosXCL$Filtro,
    K               = DialogosXCL$K,
    Semilla         = DialogosXCL$Semilla,
